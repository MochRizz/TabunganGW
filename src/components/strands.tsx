...
const buildPalette = (colors: string[]) => {
  ...
  for (let i = 0; i < MAX_COLORS; i++) {
    const hex = filled[i % filled.length] ?? filled[filled.length - 1]
...
export default function Strands({
  ...
  // Keep ref in sync with props for the animation loop
  useEffect(() => {
    propsRef.current = {
      colors,
      count,
  ...
  useEffect(() => {
    ...
    const gl = (renderer as unknown as { gl: WebGL2RenderingContext & { canvas: HTMLCanvasElement } }).gl
    ...
    if ((geometry as Record<string, unknown>).attributes) {
      const attrs = (geometry as Record<string, Record<string, unknown>>).attributes
      if (attrs.uv) {
        delete attrs.uv
      }
    }
